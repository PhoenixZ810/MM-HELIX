...
class EulerianCycleGenerator(BaseGenerator):
    """
    欧拉回路问题生成器，生成图和相关的欧拉回路问题
    """
    
    def __init__(self, output_folder="output/eulerian_cycle_problems"):
        """
        初始化生成器
    ...
    def generate(self, num_cases=10, difficulty=1, output_folder=None):
        ...
        # 调用生成欧拉回路问题的方法
        problems = EulerianCycleGenerator.generate_eulerian_cycle_problems(
            num_cases=num_cases, 
        ...
        with open(metrics_file, "w", encoding="utf-8") as f:
            json.dump({"score_function": "eulerian_cycle_evaluator"}, f, ensure_ascii=False, indent=4)
    ...
    @staticmethod
    def is_eulerian_cycle(adj_list):
        """
        判断一个无向连通图是否有欧拉回路：
          - 若所有节点度数都为偶数，则返回 True，否则 False。
        """
    ...
    def find_eulerian_cycle(adj_list):
        """
        如果图存在欧拉回路（所有节点度数为偶数、且连通），使用 Hierholzer 算法构造并返回一条欧拉回路。
        否则返回 None。
        
        返回的回路是一个顶点序列，其中首尾节点相同，例如 [0, 2, 1, 0] 表示 0->2->1->0。
        同时返回算法执行步骤的记录。
        """
        if not EulerianCycleGenerator.is_eulerian_cycle(adj_list):
            return None, []
        
        # 记录算法执行步骤
        algorithm_steps = []
        
        # 将图拷贝一份，因为构造欧拉回路会"消耗"边
        # 存储为一个 dict: node -> multiset/list of neighbors
        ...
        # 从一个非孤立顶点开始（我们这里图是连通的，所以找任意度>0的即可）
        ...
        # Hierholzer 算法
        trail = [start_node]  # 当前走的这条路
        circuit = []          # 最终构造的回路
        algorithm_steps.append(f"Step 3: Initialize trail=[{start_node}], circuit=[]")
        ...
        while trail:
            ...
            if graph_copy[current]:
                ...
                # 当前节点已无可走的边，将它弹出添加到欧拉回路
                removed = trail.pop()
                circuit.append(removed)
                algorithm_steps.append(f"Step {step_count}: Vertex {removed} has no more edges, add to circuit, current trail={trail}")
            step_count += 1
        
        # circuit 逆序存放了路径，翻转一下
        circuit.reverse()
        algorithm_steps.append(f"Step {step_count}: Reverse the circuit to get final result: {circuit}")
        
        return circuit, algorithm_steps
    ...
    @staticmethod
    def generate_solution_steps(adj_list, has_cycle, cycle=None, algorithm_steps=[]):
        """
        生成欧拉回路问题的详细解决步骤，记录判断和构造过程。
        ...
        # 创建一个段落式的解法描述
        paragraph_description = "To determine if an Eulerian cycle exists in the given graph, I'll follow these steps:"
        
        # 步骤1：检查基本条件
        steps.append("Step 1: Check basic conditions for Eulerian cycle.")
        steps.append("1.1: For a graph to have an Eulerian cycle, it must have:")
        steps.append("  - All vertices with even degrees")
        ...
        if odd_count == 0:
            ...
            steps.append("  - This means an Eulerian cycle exists")
            
            if has_cycle and cycle is not None:
                # 步骤3：显示构造的欧拉回路
                steps.append("\nStep 2: Eulerian cycle found using Hierholzer's algorithm.")
                steps.append(f"2.1: Complete cycle: {' -> '.join(map(str, cycle))}")
                steps.append(f"2.2: Cycle as list format: {cycle}")
                steps.append(f"2.3: Length of cycle: {len(cycle)}")
                ...
                steps.append("\nFinal Conclusion: The graph has an Eulerian cycle.")
                steps.append(f"Answer: {cycle}")
            else:
                steps.append("\nFinal Conclusion: The graph has an Eulerian cycle.")
                steps.append("Answer: [cycle would be constructed here]")
        else:
            steps.append(f"  - Found {odd_count} vertices with odd degree: {odd_vertices}")
            steps.append("\nFinal Conclusion: The graph does not have an Eulerian cycle because it has vertices with odd degrees.")
    ...
    @staticmethod
    def generate_eulerian_cycle_problems(num_cases=5, difficulty=1, output_folder="eulerian_cycle_problems"):
        """
        生成欧拉回路问题集
        ...
        # 创建临时实例获取参数
        temp_instance = EulerianCycleGenerator()
        ...
        for i in range(num_cases):
            # 1) 生成连通无向图
            adjacency_list = EulerianCycleGenerator.generate_random_connected_undirected_graph(
                num_nodes, 
                edge_prob, 
                seed=time.time()
            )
            ...
            # 2) 查找欧拉回路
            cycle, algorithm_steps = EulerianCycleGenerator.find_eulerian_cycle(adjacency_list)
            
            answer_field = random.choice(answer_fields)
            
            while isinstance(cycle, list) != answer_field:
                adjacency_list = EulerianCycleGenerator.generate_random_connected_undirected_graph(
                    num_nodes, 
                    edge_prob, 
                    seed=time.time()
            ...
            # 3) 生成详细的解法步骤
            solution_steps = EulerianCycleGenerator.generate_solution_steps(
                adjacency_list, 
                cycle is not None, 
                cycle, 
                algorithm_steps
            ...
            EulerianCycleGenerator.draw_and_save_graph(adjacency_list, image_path)
            
            # 5) 题目描述
            adj_str = "\n".join([f"{u}: {adjacency_list[u]}" for u in sorted(adjacency_list.keys())])
            question_text = (
                "Given the undirected, connected graph below, "
                "determine if there is an Eulerian cycle. "
                "If it exists, output the cycle as a list (e.g., [0,1,2,3,0]). If not, output 'No'."
            )
            ...
            if cycle is not None:
                answer_text = str(cycle)  # 输出回路列表格式
            else:
                answer_text = "No"
            
            # 7) 生成 JSON 条目
            with_image_entry = {
                "index": f"eulerian_cycle_with_image_{difficulty}_{i}",
                "category": "eulerian_cycle",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Eulerian cycle problems")
    ...
    parser.add_argument("--output_folder", type=str, default="eulerian_cycle_problems", help="Output folder")
    args = parser.parse_args()
    
    output_folder = args.output_folder
    os.makedirs(output_folder, exist_ok=True)
    
    generator = EulerianCycleGenerator(output_folder=output_folder)
